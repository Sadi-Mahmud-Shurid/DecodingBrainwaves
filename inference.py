...
def clean_deepseek_output(text):
    """Remove DeepSeek reasoning tokens for fair comparison"""
    import re
...
def main():
    ...
    elif "qwen" in args.model_path.lower() or "deepseek" in args.model_path.lower():
        messages = [
                {"role": "user", "content": f"You are a helpful assistant. <image> <label_string> Describe this image in one sentence:"},
            ]
    else:
        messages = [
    ...
    for batch in tqdm(test_dataloader):
        ...
        for j, output in enumerate(output_text):
            ...
            cleaned_output = clean_deepseek_output(output)
            
            print("Output generated:", cleaned_output)
            print("Expected caption:", caption_raw[j])
            batch_data[j]["Expected Caption"] = (
                caption_raw[j].replace("<s>", "").replace("</s>", "")
            )
            batch_data[j]["Generated Caption"] = cleaned_output  # Use cleaned version
